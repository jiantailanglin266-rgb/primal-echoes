...
## B8 最終品質監査（2026-09-07）
- `docs/brand/QA_REPORT.md`: 一貫性（名前・色・書体・口調）、初見 10 秒、初プレイ 3 分、細部（フォント代替・はみ出し・英語・音・プレースホルダ）、B0 のトップ 10 の Before / After、残る課題 5 件。
- 修正: 画質の自動降格通知を HUD から除去、撮影の空キャンバス保護。
- けん君が決めること: 残る課題の 2（連絡先・SNS・スタジオ名）と 3（スクリーンショット・トレーラー録画）。1（glTF モデル）は素材が要る。
